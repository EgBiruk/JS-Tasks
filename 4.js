...
// такой же длины, как и заданный массив.  У каждого объекта должно быть
//  одно поле, role, которое равно admin, если значение в заданном массиве - true,
//   и user, если значение в заданном массиве - false.
// Т.е. из
// [true, false, false]
// должно получиться
...
// 2) обойдите исходный массив с помощью цикла и, используя тернарный
//  оператор, определите значение роли (admin или user) для каждого элемента 
//  массива. Определив это значение (его лучше записывать в переменную), сразу
//   же пушьте в новый массив нужный объект.
...
for (let i = 0; i < roles.length; i++) {
    let a = (roles[i] == true) ? 'admin' : 'user';
    newRoles.push({ role: a });
}
...
//  Она принимает массив в качестве параметра
//  и возвращает новый массив, в котором содержатся
//   только положительны элементы.
...
//  значение и количество элементов.Функция создает
//  новый массив и заполняет его этим значением в указанном количестве.
// Например: из переданных значений('a', 3) получаем массив['a', 'a', 'a']


function arrayMaker(value, number) {
    let newArray = [];
...
console.log(arrayMaker('a', 3))


// Задание 6
// Определите массив, например let arr = [5, 4, 3, 8, 0].
// Создайте функцию filterFor(arr, a). Функция должна вернуть
// новый массив из элементов arr, но в нем должны содержаться элементы,
...
// Задание 7
// Написать функцию получения случайного цвета.
//  Это будет строка вида "rgb(10,55,250)"
// Здесь у вас будет две функции: одна ваша - с названием 
// getRandomRGB, а вторая - функция получения случайного числа
...
// Задание 8
// Написать функцию, которая выводит в консоль квадраты чисел 
// из заданного диапазона (от min до max), где значения min и max
// передаются в функцию как параметры.
...
getSquares(1, 25)function  isInteger(x) {
let a = (x % 0 == 0) ? true : false;
    return a;
}
...
// Задание 9
// Написать функцию isInteger(x), которая определяет, является ли
// число целым, и возвращает true либо false.
// Для проверки, целое ли число, вам понадобится 
// опеатор % (подумайте, остаток от деления на какое число
// подойдет для вашей цели)

function isInteger(x) {
    let a = (x % 1 == 0) ? true : false
    return a;
  }
...
function isInteger(n) {
    let a = (n % 1 == 0) ? 'integer' : 'decimal';
    return a;
  }

 function func(n) {
   for (let i = 1; i <= n; i += 0.5) {
    let a = (i % 1 == 0) ? 'integer' : 'decimal';
    console.log(`"${i}  ${a}"`)
   }
 }
